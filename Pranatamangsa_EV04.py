...
# ─── EV04: Klimatologi 6H-derived ────────────────────────────────────
# Basis: R30 1996–2025, data 6-jam 2 stasiun (IDW), agregasi harian.
# Format: (vpd, tcwv, cloud, cloud_aft, sun_h, sm_sh, sm_dp, sT_sh, sT_dp)
METEO_MANGSA_6H: Dict[int, Tuple] = {
     1: (1.177, 34.2,  50,    54,       11.4,  0.154, 0.335, 26.9,  26.8),
     2: (1.339, 32.1,  49,    54,       11.4,  0.132, 0.321, 27.7,  27.0),
     3: (1.498, 33.1,  53,    56,       11.4,  0.126, 0.312, 28.4,  27.2),
     4: (1.535, 36.8,  64,    64,       11.3,  0.134, 0.301, 29.1,  27.5),
     5: (0.891, 46.5,  85,    88,       11.4,  0.271, 0.291, 28.4,  27.9),
     6: (0.463, 52.0,  96,    98,       11.5,  0.379, 0.291, 26.8,  27.9),
     7: (0.400, 52.7,  97,    98,       11.8,  0.394, 0.377, 26.2,  27.2),
     8: (0.393, 52.4,  95,    96,       11.7,  0.396, 0.404, 26.2,  26.9),
     9: (0.430, 51.7,  91,    92,       11.5,  0.386, 0.403, 26.5,  26.7),
    10: (0.500, 49.9,  83,    85,       11.4,  0.360, 0.395, 26.6,  26.7),
    11: (0.698, 45.8,  66,    68,       11.3,  0.313, 0.377, 26.9,  26.8),
    12: (0.902, 39.7,  56,    60,       11.3,  0.235, 0.356, 26.7,  26.8),
}

METEO_MUSIM_6H: Dict[str, Tuple] = {
    "Katiga":   (1.299, 33.3,  51,    11.4),
    "Labuh":    (0.707, 48.1,  87,    11.3),
    "Rendheng": (0.405, 52.4,  95,    11.6),
    "Mareng":   (0.737, 44.9,  67,    11.3),
}
...
def load_interpolated_6h(csv1=DEFAULT_METEO_6H, csv2=DEFAULT_METEO_6H_2,
    ...
    if path1 is None:
        ...
        m6 = _idw_merge(_read(path1), _read(path2), w1, w2).reset_index(drop=True)

    m6["date"] = pd.to_datetime(m6["time"]).dt.normalize()
    m6["hour"] = pd.to_datetime(m6["time"]).dt.hour
...
def _meteo_mangsa_block(no: int, indent: int = 6) -> None:
    """Cetak blok klimatologi per mangsa.

    Baris [6H] dipecah menjadi 4 baris pendek agar tidak melebihi lebar
    konsol W=70. Semua variabel asli EV04 (VPD, TCWV, cloud, cloud_aft,
    Sun, SM 0-7cm, SM 28-100cm, sT 0-7cm, sT 100-255cm) tetap ditampilkan.
    """
...
def _meteo_musim_block(musim: str, indent: int = 4,
    ...
    durasi_override: bila diberikan, menggantikan durasi R30 hardcode
                     pada baris "Durasi X hari" (misal untuk skenario R10).
    Baris [6H] dipecah menjadi 2 baris pendek agar tidak melebihi W=70.
    """
...
def print_calendar(cal: List[Dict], judul: str,
    ...
    print(box_top())
    print(box_row(judul))
    print(box_row("ERA5/Land-IFS · −7.522°LS, 112.566°BT, 28 m · IDW 2 stasiun"))
    if show_meteo:
        print(box_row(f"Skenario musim: {scenario_key} — {scn_label}"))
        print(box_row("Sumber klimatologi: R30 (1996–2025) · batas mangsa ikut skenario"))
    if show_astro:
    ...
    for musim in MUSIM_ORDER:
        ...
        if show_meteo:
            _meteo_musim_block(musim, indent=2, durasi_override=dur_aktual)
            print()
...
def print_mangsa_today(tanggal: date, scenario_key: str = DEFAULT_SCENARIO) -> None:
    ...
    if kal:
        ...
        print(box_mid())
        print(box_row(""))
        print(box_row(f"  Klimatologi (sumber R30 1996–2025 · skenario {scenario_key}):"))
        ...
        if kal["no"] in METEO_MANGSA_6H:
            vpd, tcwv, cld, cld_a, sun_h, sm_sh, sm_dp, sT_sh, sT_dp = METEO_MANGSA_6H[kal["no"]]
...
def print_klimatologi_bulanan() -> None:
    ...
    # ── RINGKASAN 6H PER MUSIM (kolom 11 agar Mareng tidak terpotong) ──
    print()
    print(f"  {'RINGKASAN 6H PER MUSIM (EV04)':^{W-2}}")
    print(f"  {'VPD · TCWV · cloud · sunshine (R30 1996–2025)':^{W-2}}")
    print()
    col_w = 11
...
def print_live_nowcast() -> None:
    ...
    print(box_row("HMM 8-D (EV04) + SR-EKF Level/Tren (ARCH(1))"))
    print(box_bot())
...
def show_menu() -> None:
    print()
    print(box_top("PRANATA MANGSA — EV04 METEO(DAILY+6H)+ASTRO"))
    print(box_row("−7.52S112.56E28m · ERA5/Land IFS HRES 1940–2026 · ENSO 1993–2026"))
    print(box_row("HMM 8-D (EV04) · VSOP87D + IERS2010 · JRC_Ephemeris 2020–2029"))
    print(box_mid())
    for item in [
...
def main_loop() -> None:
    while True:
        ...
        if pilihan == "0":
            print()
            print(box_top())
            print(box_row("Terima kasih. Sampai jumpa!  — Pranata Mangsa EV04"))
            print(box_bot())
            print()
            break
