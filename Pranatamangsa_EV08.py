...
EV08: structural restructure of EV07. All calibration numbers, per-mangsa
deltas, confidence intervals, and limitation notes from EV07 are retained
...
---------------------------
  ...
  · Station P2   : ERA5/ERA5-Land/IFS-HRES Open-Meteo — daily 1940–2026,
    ...
    Interpolation: Inverse Distance Weighting (IDW, power=2)
                   per-variable p (adaptive), Haversine 3D, Q factor
                   VPD  : w1=0.4154 (P1 terrain-opt), w2=0.5846 (P2 nearest)
                   Soil : w1=0.4996 (P1 terrain-opt), w2=0.5004 (P2 nearest)
                   Temp : w1=0.4231 (P1 terrain-opt), w2=0.5769 (P2 nearest)
...
6-HOURLY METEOROLOGICAL CALIBRATION FINDINGS (EV06 → EV07)
-----------------------------------------------------------
  · Recalibration of IDW P1+P2 hourly (w1=0.3953, w2=0.6047):
    ...
    METEO_MUSIM_6H change (EV06 → EV07):
...
6-HOURLY METEOROLOGICAL CALIBRATION FINDINGS (EV07 → EV08)
-----------------------------------------------------------
  · Recalibration IDW: fixed p=2 Euclidean degree-space (EV07)
    → adaptive p per-variabel, Haversine 3D, quality factor Q (EV08).
    Tool: idw_meteorological_interpolation.py + calibrate_pranatamangsa_ev08.py
    Source: P1 & P2 hourly 2015–2026 (102,480 rows, 30 variables)
    Geometry: d_P1=4970.6m, d_P2=4027.2m, angular_sep=128° (optimal)

    IDW weight changes (EV07 fixed → EV08 adaptive):
      Class           EV07_w1  EV08_w1   Δw1   EV08_p   Q1     Q2
      surface_thermal  0.3953   0.4231  +0.028   3.50   1.30   0.85
      humidity         0.3953   0.4154  +0.020   2.99   1.20   0.90
      precipitation    0.3953   0.4457  +0.051   2.49   1.25   0.92
      soil_moisture    0.3953   0.4996  +0.104   2.50   1.35   0.80  ← MAJOR
      radiation        0.3953   0.3963  +0.001   2.45   1.10   1.00

    Key METEO_MANGSA_6H changes (max |Δ%| per field):
      vpd    : max 4.9%  — wet season Rendheng slightly higher (P2 wetter)
      tcwv   : max 1.3%  — negligible
      cloud  : max 4.2%  — Karo/Sada (dry shoulder) slightly less cloudy
      sun_h  : max 1.3%  — effectively stable
      sm_sh  : max 25.9% ⚠ Desta −26%: terrain-opt P1 significantly drier
      sm_dp  : max 55.4% ⚠ Katiga −55%: EV07 P2-dominated overestimate
      sT_sh  : max 5.6%  ⚠ Kasa +5.6°C: terrain-opt surface warmer
      sT_dp  : max 4.5%  — consistent with sT_sh shift
...
------------------------------------
  ...
  I.3  Spatial representation
       EV08 uses adaptive IDW P1+P2 hourly with per-variable power p
       optimized via temporal cross-validation (5-fold), Haversine 3D
       distance (elev scale 150 m/m), and quality factor Q per cell
       selection type:
         · VPD/humidity : w1=0.4154, w2=0.5846, p≈3.0  (Q1=1.20, Q2=0.90)
         · soil_moisture: w1=0.4996, w2=0.5004, p≈2.5  (Q1=1.35, Q2=0.80)
         · surface_temp : w1=0.4231, w2=0.5769, p≈3.5  (Q1=1.30, Q2=0.85)
       Angular sep T→P1→P2 = 128° (triangulation optimal). r_vpd = 0.90–0.98.
       sm_dp EV07 overestimated 18–55%: P2 nearest-grid was wetter with
       60% weight; EV08 Q_terrain=1.35 corrects this systematically.
       sT_sh/sT_dp: +0.5 to +1.5°C vs EV07 (terrain-opt P1 warmer,
       now correctly weighted higher for surface thermal class).
  ...
  II. METEO_MANGSA_6H UNCERTAINTY ANALYSIS (EV07)
  ...
  II.A  Type A uncertainty — Estimation precision (interannual SE)
        ...
        95% CI of VPD, representative per mangsa (EV07 IDW P1+P2):
        ...
        95% CI of sun_h, representative per mangsa (EV07 IDW P1+P2):
          Kasa:  10.84 ± 0.12 h    Kapat: 10.94 ± 0.59 h    Kapitu:   7.70 ± 0.39 h
          Karo:  11.00 ± 0.05 h    Kalima:10.11 ± 0.86 h    Kawolu:   8.47 ± 0.53 h
          Katiga:11.04 ± 0.15 h    Kanem:  8.53 ± 0.56 h    Kasanga:  9.11 ± 0.51 h
          Kasadasa: 9.62 ± 0.45 h  Desta: 10.34 ± 0.31 h    Sada:    10.34 ± 0.31 h
  ...
  II.B  Type B uncertainty — Period representation bias
        ...
        −0.03 to −0.24 kPa. Correction is NOT applied because EV07
...
IDW_POWER: float = 2.0          # legacy scalar; EV08 uses per-variable p
# EV08 adaptive IDW weights: (w1_terrain_opt, w2_nearest, power_p)
# Source: idw_meteorological_interpolation.py — Haversine 3D, Q factor
IDW_WEIGHTS_EV08: Dict[str, tuple] = {
    "surface_thermal": (0.4231, 0.5769, 3.5),
    "humidity": (0.4154, 0.5846, 2.99),
    "precipitation": (0.4457, 0.5543, 2.49),
    "radiation": (0.3963, 0.6037, 2.45),
    "cloud_cover": (0.3967, 0.6033, 1.99),
    "pressure": (0.3965, 0.6035, 3.16),
    "wind_speed": (0.3965, 0.6035, 2.45),
    "wind_direction": (0.3966, 0.6034, 2.0),
    "soil_moisture": (0.4996, 0.5004, 2.5),
    "weather_code": (0.422, 0.578, 1.5),
}
...
def astro_delta_str(key: str) -> str:
    ...
    if not ev.same_event:
        return f"Δ={ev.delta:+.1f} hr (definisi berbeda)"
    if abs(ev.delta) < 0.5:
        return f"Δ={ev.delta:+.1f} hr (konsisten tradisional)"
    direction = "lebih awal" if ev.delta < 0 else "lebih lambat"
    return f"Δ={ev.delta:+.1f} hr ({abs(ev.delta):.1f} hr {direction})"
...
# EV08: IDW Adaptive (per-variable p, Haversine 3D, quality factor Q).
# Source: idw_meteorological_interpolation.py | 102,480 hourly timestep.
# Fields: (vpd, tcwv, cloud, cloud_aft, sun_h, sm_sh, sm_dp, sT_sh, sT_dp)
METEO_MANGSA_6H: Dict[int, Tuple] = {
     1: (1.345, 35.1, 48, 54, 10.86, 0.127, 0.172, 28.4, 28.0),  # Δ[-0.007, -0.100, -1.000, -1.000, +0.020, -0.027, -0.163, +1.500, +1.200]
     2: (1.542, 33.7, 46, 50, 11.01, 0.112, 0.151, 29.2, 28.1),  # Δ[-0.009, +0.000, -2.000, -1.000, +0.010, -0.020, -0.170, +1.500, +1.100]
     3: (1.692, 35.8, 55, 60, 11.04, 0.114, 0.139, 29.9, 28.2),  # Δ[+0.002, +0.000, +0.000, +0.000, +0.000, -0.012, -0.173, +1.500, +1.000]
     4: (1.602, 39.7, 65, 66, 10.94, 0.152, 0.140, 30.3, 28.5),  # Δ[+0.001, +0.100, +0.000, +0.000, +0.000, +0.018, -0.161, +1.200, +1.000]
     5: (1.169, 46.8, 78, 79, 10.06, 0.224, 0.146, 29.5, 28.9),  # Δ[-0.012, +0.200, +1.000, +1.000, -0.050, -0.047, -0.145, +1.100, +1.000]
     6: (0.627, 52.6, 91, 91, 8.51, 0.316, 0.230, 27.7, 28.7),  # Δ[-0.002, +0.200, +0.000, +0.000, -0.020, -0.063, -0.061, +0.900, +0.800]
     7: (0.480, 54.1, 92, 93, 7.80, 0.349, 0.306, 26.7, 28.0),  # Δ[+0.013, +0.400, +0.000, +0.000, +0.100, -0.045, -0.071, +0.500, +0.800]
     8: (0.491, 52.9, 88, 90, 8.53, 0.352, 0.328, 26.9, 27.5),  # Δ[+0.023, +0.200, +0.000, +0.000, +0.060, -0.044, -0.076, +0.700, +0.600]
     9: (0.547, 52.5, 84, 87, 9.19, 0.341, 0.323, 27.3, 27.4),  # Δ[+0.015, +0.300, +0.000, +0.000, +0.080, -0.045, -0.080, +0.800, +0.700]
     10: (0.676, 49.8, 77, 79, 9.64, 0.308, 0.303, 27.7, 27.3),  # Δ[+0.020, +0.300, +0.000, +0.000, +0.020, -0.052, -0.092, +1.100, +0.600]
     11: (0.949, 45.8, 62, 66, 10.34, 0.232, 0.256, 28.3, 27.6),  # Δ[+0.000, +0.600, -1.000, +0.000, +0.000, -0.081, -0.121, +1.400, +0.800]
     12: (1.022, 42.9, 59, 63, 10.42, 0.199, 0.214, 28.1, 27.9),  # Δ[+0.001, -0.300, -2.000, -2.000, +0.080, -0.036, -0.142, +1.400, +1.100]
}

METEO_MUSIM_6H: Dict[str, Tuple] = {
    "Katiga":   (1.496, 35.0, 50, 10.9),
    "Labuh":    (1.042, 47.4, 80,  9.6),
    "Rendheng": (0.484, 53.0, 89,  8.3),
    "Mareng":   (0.902, 45.4, 66, 10.1),
}
...
#
# The VPD coefficients follow from a linearisation of the Tetens equation
...
# 8-D EV07 model : [rain_30d, wb_30d, sm_30d, rh_30d,
...
def build_calendar_terkalibrasi(
    ...
    for i, no in enumerate(sorted_nos):
        ...
        nxt_dp = (new_dopy[nxt_no] if nxt_no != 1
                  else 365 + new_dopy[1])
...
def _idw_weights(
    ...
    """Inverse-distance-weighting coefficients (normalised)."""
...
def _aggregate_to_daily(
    ...
    """Vectorised sub-hourly → daily aggregation (EV08 revision).
...
def _prep_8d_from_daily(df: "pd.DataFrame") -> Optional["pd.DataFrame"]:
    """Construct the 8-D feature matrix used by the EV07 HMM."""
...
def live_nowcast(
    ...
    if df_8d is not None and len(df_8d) >= 60:
        ...
        hmm_mode = "8-D (EV07: +TCWV+DTR+cloud+SM-dalam)"
        df_trend = df_8d
    else:
...
# ══════════════════════════════════════════════════════════════════════════
# Section 14 · Climate blocks (unified)
# ══════════════════════════════════════════════════════════════════════════
...
def print_astro_calib_table() -> None:
    print()
    print(box_top())
    print(box_row("KALIBRASI ASTRONOMIS — EV08"))
...
def print_klimatologi_bulanan() -> None:
    ...
    print(f"  {'RINGKASAN 6H PER MUSIM (EV07)':^{W - 2}}")
...
def print_live_nowcast() -> None:
    ...
    print(box_row("HMM 8-D (EV08) + SR-EKF Level/Tren (ARCH(1))"))
    print(box_bot())
...
def show_menu() -> None:
    print(); print(box_top("PRANATA MANGSA — EV08 METEO(DAILY+6H)+ENSO+IOD+ASTRO"))
    ...
    print(box_row("HMM 8-D (EV07) · VSOP87D + IERS2010 · JRC_Ephemeris 2020–2029"))
    print(box_mid())
...
def main_loop() -> None:
    while True:
        ...
        if pilihan == "0":
            print(); print(box_top())
            print(box_row("Terima kasih. Sampai jumpa! — Pranata Mangsa EV08"))
            print(box_bot()); print()
            break
...
def _build_argparser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        prog="pranatamangsa",
        description="Calibrated Pranata Mangsa calendar (EV08).",
    )
