...
-------------------------------------------------------------------------
  ...
  · HMM 4-D & 8-D direkalibrasi pada 31-yr (1995–2025, n=11.309).
    Konvergensi BW: 62 iter (4-D), 137 iter (8-D).
    logL = −706,71 (4-D) / −630,18 (8-D) — perbaikan fit vs EV06b (11-yr).      
...
DEFAULT_HOURLY_CSV_P1  = (
    "open-meteo-7.49S112.54E28m_hourly10yr.csv",
    "/storage/emulated/0/Download/-7.49S112.54E28m_hourly19952014.csv",
)
DEFAULT_HOURLY_CSV_P2  = (
    "open-meteo-7.56S112.56E28m_hourly10yr.csv",
    "/storage/emulated/0/Download/-7.56S112.56E28m_hourly19952014.csv",
)
...
#
# ── Rekalibrasi 31-tahun (1995–2025) · n = 11.309 sampel harian ──────────
# Rekalibrasi 31-yr: 1995–2025, n=11.309, logL=−706,71 (4-D) / −630,18 (8-D).
# Sumber: P1 (−7.49S 112.54E 28m) + P2 (−7.56S 112.56E 28m), hourly
#   1995–2026 → agregasi harian → IDW w1=0.4154 / w2=0.5846 →
#   rolling-30d (sum untuk rain_30d & wb_30d; mean untuk sisanya).
# Normalisasi μ/σ = populasi (ddof=0), satuan ASLI (bukan z-score).
# Baum-Welch EM, K=4 Gaussian full-covariance, tol=1e-7, reg_covar=1e-4,
#   init quantile-split deterministik (seed=42), tanpa smoothing A.
# State relabeling: urut rain_30d ascending, tie-break sm_30d.
#
#   logL 4-D = −706,713  (konvergen @ iter 62)
#   logL 8-D = −630,179  (konvergen @ iter 137)
#
# Anchor π = [0, 1, 0, 0] dipertahankan dari EV09 (state awal = transisi
# kering), karena π BW hanya mencerminkan hari pertama sampel (1995-07-06)
# dan tidak bermakna untuk nowcast operasional.

# ── 4-D legacy ───────────────────────────────────────────────────────────
HMM_T_pi    = [0.0, 1.0, 0.0, 0.0]
HMM_T_A     = [
    [0.99202, 0.00613, 0.00000, 0.00186],
    [0.00761, 0.98129, 0.00034, 0.01077],
    [0.00000, 0.00811, 0.99189, 0.00000],
    [0.00434, 0.00089, 0.00954, 0.98523],
]
HMM_T_means = [
    [-1.01054, -1.04235, -1.25739, -1.21514],
    [-0.45490, -0.40156, +0.25504, +0.06232],
    [+1.24563, +1.24336, +1.12007, +1.06164],
    [+0.54246, +0.53539, +0.29208, +0.51124],
]
HMM_T_covs = [
    [  # state 0 — Katiga
        [0.01251, 0.01438, 0.01913, 0.03151],
        [0.01438, 0.02480, 0.04118, 0.07175],
        [0.01913, 0.04118, 0.13510, 0.15725],
        [0.03151, 0.07175, 0.15725, 0.26794],
    ],
    [  # state 1 — Labuh/Mareng kering
        [0.17057, 0.15797, 0.10785, 0.09931],
        [0.15797, 0.15183, 0.12774, 0.12473],
        [0.10785, 0.12774, 0.28795, 0.28934],
        [0.09931, 0.12473, 0.28934, 0.31432],
    ],
    [  # state 2 — Rendheng
        [0.27512, 0.26357, 0.02283, 0.05529],
        [0.26357, 0.25377, 0.02231, 0.05550],
        [0.02283, 0.02231, 0.00324, 0.00702],
        [0.05529, 0.05550, 0.00702, 0.02316],
    ],
    [  # state 3 — Labuh/Mareng basah
        [0.33296, 0.32774, 0.18901, 0.16874],
        [0.32774, 0.32495, 0.19505, 0.17526],
        [0.18901, 0.19505, 0.20989, 0.15954],
        [0.16874, 0.17526, 0.15954, 0.15979],
    ],
]
HMM_T_mu    = [218.88785, 95.58441, 0.27638, 78.19585]
HMM_T_sd    = [196.49369, 213.29929, 0.10645, 10.12722]

# ── 8-D EV06b ────────────────────────────────────────────────────────────
HMM_T8_pi    = [0.0, 1.0, 0.0, 0.0]
HMM_T8_A     = [
    [0.99138, 0.00862, 0.00000, 0.00000],
    [0.00035, 0.98879, 0.00914, 0.00172],
    [0.00000, 0.00027, 0.99211, 0.00763],
    [0.00951, 0.00454, 0.00137, 0.98458],
]
HMM_T8_means = [
    [-1.02706, -1.02553, -0.97161, -1.04779,
     -1.33087, +1.25598, -1.12479, -0.44278],
    [-0.51537, -0.56020, -0.85135, -0.70276,
     -0.18947, +0.41410, -0.32168, -1.18965],
    [+1.17683, +1.16993, +0.97772, +0.94744,
     +0.84053, -0.87520, +1.02016, +0.88147],
    [-0.11200, -0.04422, +0.59150, +0.53776,
     +0.37907, -0.52538, +0.01195, +0.57909],
]
HMM_T8_covs = [
    [  # state 0 — Katiga
        [0.00720, 0.00884, 0.02193, 0.02083, 0.02823, -0.02990, 0.01290, 0.00640],
        [0.00884, 0.01768, 0.04877, 0.05790, 0.04471, -0.06813, 0.01467, 0.03484],
        [0.02193, 0.04877, 0.19460, 0.19477, 0.12429, -0.21823, 0.02789, 0.15370],
        [0.02083, 0.05790, 0.19477, 0.24019, 0.13880, -0.25969, 0.01461, 0.17211],
        [0.02823, 0.04471, 0.12429, 0.13880, 0.18067, -0.19316, 0.06479, 0.06592],
        [-0.02990, -0.06813, -0.21823, -0.25969, -0.19316, 0.31342, -0.05119, -0.16912],
        [0.01290, 0.01467, 0.02789, 0.01461, 0.06479, -0.05119, 0.18852, 0.00027],
        [0.00640, 0.03484, 0.15370, 0.17211, 0.06592, -0.16912, 0.00027, 0.17092],
    ],
    [  # state 1 — Labuh/Mareng kering
        [0.33492, 0.34326, 0.37651, 0.39481, 0.34981, -0.26549, 0.43258, 0.10660],
        [0.34326, 0.35863, 0.40104, 0.43776, 0.38040, -0.31731, 0.44355, 0.13275],
        [0.37651, 0.40104, 0.55101, 0.56850, 0.47695, -0.45181, 0.51960, 0.28279],
        [0.39481, 0.43776, 0.56850, 0.70052, 0.60518, -0.60041, 0.56058, 0.31047],
        [0.34981, 0.38040, 0.47695, 0.60518, 0.69473, -0.52063, 0.59326, 0.19369],
        [-0.26549, -0.31731, -0.45181, -0.60041, -0.52063, 0.64724, -0.37974, -0.33373],
        [0.43258, 0.44355, 0.51960, 0.56058, 0.59326, -0.37974, 0.74808, 0.11899],
        [0.10660, 0.13275, 0.28279, 0.31047, 0.19369, -0.33373, 0.11899, 0.40510],
    ],
    [  # state 2 — Rendheng
        [0.27009, 0.26270, 0.06903, 0.10113, 0.07934, -0.05553, 0.07338, 0.12809],
        [0.26270, 0.25679, 0.06849, 0.10168, 0.08127, -0.05998, 0.07688, 0.12773],
        [0.06903, 0.06849, 0.06182, 0.04711, 0.01098, -0.01660, 0.01122, 0.11806],
        [0.10113, 0.10168, 0.04711, 0.06507, 0.04162, -0.03786, 0.03905, 0.10375],
        [0.07934, 0.08127, 0.01098, 0.04162, 0.06701, -0.04913, 0.05232, 0.03153],
        [-0.05553, -0.05998, -0.01660, -0.03786, -0.04913, 0.07500, -0.04732, -0.05337],
        [0.07338, 0.07688, 0.01122, 0.03905, 0.05232, -0.04732, 0.08855, 0.03430],
        [0.12809, 0.12773, 0.11806, 0.10375, 0.03153, -0.05337, 0.03430, 0.29890],
    ],
    [  # state 3 — Labuh/Mareng basah
        [0.33906, 0.31620, 0.06468, 0.14555, 0.34130, -0.19762, 0.34094, -0.01566],
        [0.31620, 0.29734, 0.06623, 0.14345, 0.32146, -0.19002, 0.32154, -0.00830],
        [0.06468, 0.06623, 0.10372, 0.09435, 0.04035, -0.05006, 0.06072, 0.12436],
        [0.14555, 0.14345, 0.09435, 0.13133, 0.15709, -0.12442, 0.16431, 0.08686],
        [0.34130, 0.32146, 0.04035, 0.15709, 0.48387, -0.27635, 0.42712, -0.06268],
        [-0.19762, -0.19002, -0.05006, -0.12442, -0.27635, 0.19528, -0.25770, 0.00061],
        [0.34094, 0.32154, 0.06072, 0.16431, 0.42712, -0.25770, 0.45226, -0.03520],
        [-0.01566, -0.00830, 0.12436, 0.08686, -0.06268, 0.00061, -0.03520, 0.20793],
    ],
]
HMM_T8_mu = [218.88785, 95.58441, 0.27638, 78.19585,
             43.99426, 8.61157, 71.06830, 0.27758]
HMM_T8_sd = [196.49369, 213.29929, 0.10645, 10.12722,
             8.19729, 1.94966, 17.86807, 0.09637]
...
def find_data_file(filename: str) -> Optional[str]:
    """Search cwd, script dir, and absolute paths."""
    if not filename:
        return None
...
def load_interpolated_6h(
    ...
    def _per_station(hourly_paths, six_paths):
        if isinstance(hourly_paths, str):
            hourly_paths = [hourly_paths]
        if isinstance(six_paths, str):
            six_paths = [six_paths]
        ...
        for hp in hourly_paths:
            ...
            if p is None:
                continue
            df_h = _read_openmeteo_csv(p)
            daily.append(_aggregate_to_daily(
                df_h, aft_slots=tuple(range(12, 18)), sec_per_slot=3600))
            del df_h
        if daily:
        ...
        for sp in six_paths:
            ...
            if p is None:
                continue
            df_6 = _read_openmeteo_csv(p)
            daily.append(_aggregate_to_daily(
                df_6, aft_slots=(12, 18), sec_per_slot=21600))
            del df_6
        if daily:
            return (pd.concat(daily, ignore_index=True)
                      .drop_duplicates("time")
...
def live_nowcast(
    ...
    out: Dict = {
        ...
        "data_start":         df_8d["time"].iloc[0].date() if df_8d is not None and len(df_8d) else df["time"].iloc[0].date(),
        "hmm_mode":           hmm_mode,
        "has_6h":             has_6h,
    }
