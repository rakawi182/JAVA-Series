...
def _meteo_mangsa_block_dopy(dopy_s: float, dopy_e: float,
    ...
    """
    m, m6h = meteo_for_dopy_range(dopy_s, dopy_e, enso_phase=enso_phase)
    if m is None:
        return
...
def _meteo_musim_block(musim: str, indent: int = 4,
    ...
    # ── Jalur (1): interpolasi dopy + koreksi ENSO ──────────────────
    if dopy_s is not None and dopy_e is not None:
        m, m6h = meteo_for_dopy_range(dopy_s, dopy_e, enso_phase=enso_phase)
        if m is not None:
            hj, hj_d, hhr, et0, wb, sm, rh, tx, tn, angin, rad = m
...
def print_calendar(cal: List[Dict], judul: str,
    ...
    # Peta skenario → fase ENSO untuk koreksi klimatologi
    ...
    if kal:
        ...
        if kal["no"] in METEO_MANGSA:
            ...
            print(box_row(f"  Suhu udara  : "
                          f"{_fmt_suhu(tx, tn, kal.get('dopy_start'), kal.get('dopy_end'))}"))
...
def show_menu() -> None:
    print()
    print(box_top("PRANATA MANGSA — EV05 METEO(DAILY+6H)+ASTRO"))
