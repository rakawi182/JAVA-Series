...
# -*- coding: utf-8 -*-
"""
Pranatamangsa_Reanalysis_Ev06b.py
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
KALENDER PRANATA MANGSA — METEO(DAILY+6H) + ENSO + IOD + ASTRONOMI
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
SUMBER DATA METEOROLOGI
  ...
  · ENSO Niño3.4 mingguan — AVISO/DUACS (SLA) + NOAA OISST v2.1 (SST),
                    1993–2026
  · IOD DMI bulanan — JMA/BoM, 1950–2025
...
TEMUAN KALIBRASI ASTRONOMIS (EV02 vs EV01, diwarisi EV03/EV04/EV05/EV06/EV06b)
...
TEMUAN KALIBRASI METEOROLOGI 6H (EV05 → EV06)
  · Rekalibrasi METEO_MANGSA_6H dari ERA5/Land 1H (hourly) P1:
    ...
    Metode cloud: rata² 24 jam (total) dan rata² jam 12–18 WIB (cloud_aft)
    Metode tcwv : rata² 24 jam

    Penyebab perubahan vs EV05:
    1. VPD NAIK (+0.05–+0.25 kPa): EV05 pakai 6H snapshot (jam 00,06,12,18)
       sehingga malam (VPD rendah) dapat bobot 50%. Hourly 24h lebih seimbang.
    2. sun_h TURUN (−0.1–−1.6 jam): EV05 pakai interpolasi 6H → overestimate
       di musim hujan saat langit berubah cepat. Hourly sum lebih presisi.
    3. cloud TURUN (−1–−8%): cloud_aft (12–18) lebih representatif vs 6H
       yang snapshot jam 12 saja bisa bias awan pagi.
    4. tcwv NAIK (+0.5–+3.7 kg/m²): resolusi 1H lebih halus dari 4 titik/hari.

    Perubahan vpd per mangsa (EV05→EV06):
    ...
    Perubahan sun_h per mangsa (EV05→EV06):
    ...
    → Koreksi terbesar di Rendheng (Kapitu/Kawolu): EV05 overestimate
      hingga 1.6 jam/hari karena variabilitas awan tinggi tak tertangkap 6H.

    Perubahan METEO_MUSIM_6H (EV05→EV06):
...
TEMUAN KALIBRASI METEOROLOGI 6H (EV04 → EV05)
  · Recompute METEO_MANGSA_6H field sun_h & vpd:
    ...
    → Diarsipkan; nilai final adalah rekalibrasi EV06b di atas.
...
CATATAN PENEMPATAN CIRI (koreksi EV03, diwarisi EV04/EV05/EV06)
...
# ── Koefisien fisis untuk koreksi delta (EV06b) ────────────────────
# Semua nilai dikalibrasi terhadap data hourly P1 2015–2025. Lihat
# §7 untuk justifikasi dan interval kepercayaan.
...
DATA_ATTRIBUTION: Dict[str, Dict[str, Optional[str]]] = {
    ...
    "ecmwf_ifs": {
        "nama": "ECMWF IFS (HRES)",
        "deskripsi": "Model NWP global, 9 km, 2016–sekarang (Cy41r2+)",
        "institusi": "European Centre for Medium-Range Weather Forecasts",
        "sitasi": "ECMWF. (2024). IFS Documentation CY49r1.",
        "doi": None,
        "lisensi": "CC-BY 4.0",
        "catatan": ("Dikumpulkan oleh Open-Meteo dari run IFS 0z/6z/12z/18z "
                    "sejak Cy41r2 (Maret 2016)."),
    },
    "open_meteo": {
        "nama": "Open-Meteo",
...
CIRI: Dict[int, str] = {
    ...
    6: ("Weluku/Orion Acronychal Rise (pertama terlihat di senja): ~5 Des "
        ...
        "Hujan lebat. Menabur benih padi. Solstis Desember 21 Des "
        "(dopy≈182.7) juga jatuh di Kanem, bukan di Kapitu."),
...
def build_ciri(scenario_key: str, mangsa_no: int,
    ...
    di-wrap rapi oleh :func:`textwrap.fill`.

    Parameters
    ----------
    scenario_key : str
        Kunci skenario. Hanya dipakai untuk *namespace*; penempatan astro
        murni berdasarkan rentang dopy.
    mangsa_no : int
...
# ══════════════════════════════════════════════════════════════════════
# §6  KLIMATOLOGI EMPIRIS (R30 1996–2025)
#     METEO_MANGSA    : daily P1+P2 IDW (w1=0.395, w2=0.605)
#     METEO_MANGSA_6H : HOURLY P1+P2 IDW (2015–2025, overlap 102.480 jam)
#                       direkalibrasi EV06b — lihat header modul
# ══════════════════════════════════════════════════════════════════════
...
# ── EV06b: Klimatologi 6H-derived dari HOURLY P1+P2 IDW ────────────
# Basis: R30 1996–2025. Sumber: ERA5/Land 1H P1 + P2, masing-masing
#        diagregasi harian pada resolusi 1 jam, lalu IDW-merge dengan
#        bobot w1=0.3953 (P1), w2=0.6047 (P2). Verifikasi:
#        jarak P1–P2 = 8.05 km, overlap 102.480 jam (2015-01-01 →
#        2026-09-09), korelasi r_vpd 0.90–0.98, r_sun 0.96–0.99.
# Metode:
#   · VPD      : rata² 24 jam (bukan 6H snapshot)
#   · sun_h    : sum(sunshine_duration)/3600 per hari, lalu rata² mangsa
#   · tcwv     : rata² 24 jam
#   · cloud    : rata² 24 jam
#   · cloud_aft: rata² jam 12–18 WIB (periode konvektif siang)
# sm_sh, sm_dp, sT_sh, sT_dp tetap dari EV05 (6H IDW P1+P2).
...
IOD_DELTA: Dict[str, Dict[int, ClimateDelta]] = {
    ...
    "nIOD": {
        ...
        5: ClimateDelta(-0.487, -0.064, +1.359, -0.112, -0.319, +1.964),
    },
}
IOD_DELTA["NETRAL"] = {}
...
# ══════════════════════════════════════════════════════════════════════
# §8  INTERPOLASI KLIMATOLOGI LINTAS-SKENARIO
# ══════════════════════════════════════════════════════════════════════
...
def meteo_for_dopy_range(
    ...
    Sumber 6H: :data:`METEO_MANGSA_6H` direkalibrasi EV06b dari IDW
    P1+P2 hourly (w1=0.3953, w2=0.6047, 2015–2025).
    """
    weights: Dict[int, float] = {}
    for no, (rs, re) in R30_DOPY_RANGES.items():
        ovl = max(0.0, min(dopy_e, re) - max(dopy_s, rs))
...
# ══════════════════════════════════════════════════════════════════════
# §9  PARAMETER HMM (4-D LEGACY & 8-D EV04/EV05)
# ══════════════════════════════════════════════════════════════════════
#
# Dua set parameter:
...
# ══════════════════════════════════════════════════════════════════════
# §11  NOWCAST — HMM + SR-EKF + INTEGRASI 6H/HOURLY
# ══════════════════════════════════════════════════════════════════════
...
DEFAULT_METEO_HOURLY = "open-meteo-7.49S112.54E28m_hourly10yr.csv"   # P1 hourly
DEFAULT_METEO_HOURLY_2 = "open-meteo-7.56S112.56E28m_hourly10yr.csv" # P2 hourly (EV06b)
...
def _log_mvn(X, mean, cov):
    ...
    if HAS_SCIPY:
        try:
            ...
            return multivariate_normal_gen().logpdf(X, mean=mean, cov=cov_arr)
        except Exception:
            pass
    d    = len(mean)
...
def load_interpolated_6h(
    ...
    hourly_csv2: str = DEFAULT_METEO_HOURLY_2,
) -> Optional["pd.DataFrame"]:
    """Muat data sub-harian, agregasi harian per stasiun, lalu IDW-merge.

    Prioritas sumber per stasiun:

    - **P1**: ``hourly_csv`` > ``csv1`` (6H fallback).
    - **P2**: ``hourly_csv2`` > ``csv2`` (6H fallback).

    EV06b memakai P2 hourly sebagai prioritas — sebelumnya P2 hanya 6H.
    Dengan tersedianya kedua stasiun pada resolusi 1 jam, agregasi harian
    masing-masing stasiun diikuti IDW-merge pada tingkat harian
    mereproduksi metodologi yang sama seperti :func:`load_interpolated_meteo`.

    Catatan EV06b
    -------------
    Untuk field (vpd, tcwv, cloud, cloud_aft, sun_h), tabel
    :data:`METEO_MANGSA_6H` **tidak** dihitung ulang dari fungsi ini.
    Nilai direkalibrasi secara offline dari IDW P1+P2 hourly (script
    terpisah). Fungsi ini hanya menyediakan DataFrame 6H untuk
    kebutuhan nowcast runtime — kolom yang di-merge ke ``df`` daily.

    Returns
    -------
    pd.DataFrame
        Kolom harian: tcwv, dtr, cloud_mean, cloud_aft, sm28_100,
        sunshine_h, sw_rad_MJ, vpd.
    """
    if not HAS_PANDAS:
        return None
    path_h1 = find_data_file(hourly_csv)
    path_h2 = find_data_file(hourly_csv2)
    path1 = find_data_file(csv1)
    path2 = find_data_file(csv2)

    use_hourly_p1 = path_h1 is not None
    use_hourly_p2 = path_h2 is not None
    if not (use_hourly_p1 or use_hourly_p2 or path1 or path2):
        return None
    w1, w2 = _idw_weights(lat_t, lon_t,
                          [(LAT_P1, LON_P1), (LAT_P2, LON_P2)])
    ...
    agg1 = None
    if use_hourly_p1:
        agg1 = _aggregate_to_daily(_read_openmeteo_csv(path_h1),
    ...
    agg2 = None
    if use_hourly_p2:
        agg2 = _aggregate_to_daily(_read_openmeteo_csv(path_h2),
                                   aft_slots=list(range(12, 18)),
                                   sec_per_slot=3600)
...
def live_nowcast(
    ...
    meteo_hourly: str = DEFAULT_METEO_HOURLY,
    meteo_hourly2: str = DEFAULT_METEO_HOURLY_2,
...
# ══════════════════════════════════════════════════════════════════════
# §12  TAMPILAN — BLOK KLIMATOLOGI & KALENDER
# ══════════════════════════════════════════════════════════════════════
...
def print_astro_calib_table() -> None:
    ...
    print()
    print(box_top())
    print(box_row("KALIBRASI ASTRONOMIS — EV06b"))
...
def print_calendar(
    ...
    print(box_row("−7.5220°LS, 112.5661°BT, 28 m · Daily+6H IDW P1+P2"))
    if show_meteo:
...
def print_klimatologi_bulanan() -> None:
    ...
    print(box_row("ERA5/Land-IFSHRES · −7.522°LS 112.566°BT · 28 m  [IDW P1+P2]"))
    print(box_mid())
...
def show_menu() -> None:
    ...
    print()
    print(box_top("PRANATA MANGSA — EV06b METEO(DAILY+HOURLY)+ENSO+IOD+ASTRO"))
...
def main_loop() -> None:
    ...
    while True:
        ...
        if pilihan == "0":
            print()
            print(box_top())
            print(box_row("Terima kasih. Sampai jumpa!  — Pranata Mangsa EV06b"))
            print(box_bot())
            print()
            break
...
def _build_argparser() -> argparse.ArgumentParser:
    ...
    ap = argparse.ArgumentParser(
        prog="pranatamangsa",
        description=("Pranata Mangsa EV06b — kalender pertanian tropis "
