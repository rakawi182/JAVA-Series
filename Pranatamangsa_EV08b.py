...
EV07: structural restructure of EV06b. All calibration numbers, per-mangsa
...
---------------------------
  ...
  · Station P2   : ERA5/ERA5-Land/IFS-HRES Open-Meteo — daily 1940–2026,
    ...
    Interpolation: Gradient-Enhanced IDW (GIDW, variable-specific p & α)
                   VPD:  w1=0.4419 (P1), w2=0.5581 (P2)  p=0.976  α=0.823
                   TCWV: w1=0.4374 (P1), w2=0.5626 (P2)  p=1.033  α=0.907
                   Mean: w1≈0.442,        w2≈0.558
...
------------------------------------
  ...
  I.3  Spatial representation
       EV07 uses GIDW P1+P2 hourly with variable-specific weights:
...
# GIDW effective weights for live blending (mean across variables,
# from met_idw_blender.py GIDW calibration 2015–2025).
# Per-variable p & α are in the EV07 calibration notes above.
_GIDW_W1: float = 0.4417    # terrain_optimized (P1)
...
# EV07-GIDW: Gradient-Enhanced IDW P1+P2 hourly, 2015–2025 (11 yr).
# Tuple: (vpd, tcwv, cloud, cloud_aft, sun_h, sm_sh, sm_dp, sT_sh, sT_dp)
# Method: variable-specific p & α from variogram; terrain-quality Q factors.
# sm_dp corrected vs EV06b (raw data: S1 mean 0.244, S2 mean 0.204 m³/m³).
METEO_MANGSA_6H: Dict[int, Tuple] = {
     1: (1.244, 37.2, 50, 56, 10.78, 0.149, 0.176, 28.0, 27.7),  # Kasa     Δvpd=−0.108
     2: (1.447, 34.7, 49, 55, 10.95, 0.115, 0.157, 28.7, 27.8),  # Karo     Δvpd=−0.104
     3: (1.645, 34.8, 52, 57, 11.01, 0.110, 0.141, 29.6, 28.0),  # Katiga   Δvpd=−0.045
     4: (1.646, 38.5, 61, 63, 11.00, 0.134, 0.133, 30.2, 28.3),  # Kapat    Δvpd=+0.045
     5: (1.408, 42.7, 73, 74, 10.83, 0.187, 0.141, 30.0, 28.7),  # Kalima   Δvpd=+0.227
     6: (0.812, 51.5, 87, 87,  8.81, 0.279, 0.182, 28.5, 28.9),  # Kanem    Δvpd=+0.183
     7: (0.563, 53.3, 92, 92,  8.17, 0.327, 0.265, 27.2, 28.5),  # Kapitu   Δvpd=+0.096
     8: (0.478, 54.0, 90, 91,  8.19, 0.345, 0.313, 26.7, 27.9),  # Kawolu   Δvpd=+0.010
     9: (0.536, 52.8, 87, 89,  8.85, 0.336, 0.319, 27.1, 27.5),  # Kasanga  Δvpd=+0.004
    10: (0.604, 51.8, 79, 82,  9.42, 0.321, 0.305, 27.6, 27.4),  # Kasadasa Δvpd=−0.052
    11: (0.856, 47.3, 70, 73, 10.19, 0.254, 0.267, 28.2, 27.5),  # Desta    Δvpd=−0.093
    12: (1.010, 44.5, 60, 64, 10.38, 0.201, 0.214, 28.3, 27.9),  # Sada     Δvpd=−0.011
}

METEO_MUSIM_6H: Dict[str, Tuple] = {
    "Katiga":   (1.407, 35.9, 50, 10.89),  # Δvpd=−0.089  Δsun=−0.01
    "Labuh":    (1.201, 45.6, 76,  9.96),  # Δvpd=+0.159  Δsun=+0.36
    "Rendheng": (0.532, 53.3, 90,  8.36),  # Δvpd=+0.048  Δsun=+0.06
    "Mareng":   (0.859, 47.2, 68, 10.07),  # Δvpd=−0.043  Δsun=−0.03
}
...
def _idw_weights(
    ...
    pre-computed mean GIDW effective weights (_GIDW_W1, _GIDW_W2) derived
    from the full variable-specific calibration (met_idw_blender.py).
    ...
    # Fast path: standard 2-station P1+P2 → use calibrated GIDW weights
    if (len(coords) == 2
            and coords[0] == STATION_P1
            and coords[1] == STATION_P2):
        return [_GIDW_W1, _GIDW_W2]
...
def print_astro_calib_table() -> None:
    print()
    print(box_top())
    print(box_row("KALIBRASI ASTRONOMIS — EV07"))
...
def print_live_nowcast() -> None:
    ...
    print(box_row("HMM 8-D (EV07) + SR-EKF Level/Tren (ARCH(1))"))
    print(box_bot())
...
def show_menu() -> None:
    print(); print(box_top("PRANATA MANGSA — EV07 GIDW+METEO(DAILY+6H)+ENSO+IOD+ASTRO"))
...
def main_loop() -> None:
    while True:
        ...
        if pilihan == "0":
            print(); print(box_top())
            print(box_row("Terima kasih. Sampai jumpa! — Pranata Mangsa EV07"))
            print(box_bot()); print()
            break
...
def _build_argparser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(
        prog="pranatamangsa",
        description="Calibrated Pranata Mangsa calendar (EV07).",
    )
